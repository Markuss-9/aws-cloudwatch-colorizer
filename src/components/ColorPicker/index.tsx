...
const ColorPicker = ({
  ...
  return (
    <div className="z-[1000] flex flex-col items-center bg-surface-card rounded-lg p-3 w-[240px]">
      ...
      <div className="mt-3 w-full">
        <div className="flex bg-surface-hover rounded p-0.5 mb-2">
          {FORMATS.map((f) => (
            <button
              key={f.key}
        ...
        {format === 'rgba' && (
          <div className="flex gap-1">
            {(['r', 'g', 'b'] as const).map((f) => (
              ...
              >
                <span className="text-[9px] text-gray-400 uppercase">{f}</span>
                <NumberInput
                  min={0}
                  max={255}
                  value={rgbaInputs[f]}
                  onChange={(e) => handleRgbaFieldChange(f, e.target.value)}
                  className="w-full bg-surface-hover text-white text-xs text-center border border-border-form rounded py-1 font-mono outline-none focus:border-brand transition-colors"
                />
              </div>
            ))}
        ...
        {format === 'hex' && (
          <div className="flex flex-col items-center gap-0.5">
            <span className="text-[9px] text-gray-400 uppercase">#RRGGBB</span>
            <input
              type="text"
              value={hexInput}
      ...
      {onClose && (
        <button
          onClick={onClose}
          className="mt-3 text-xs text-white bg-brand hover:bg-brand-hover cursor-pointer border-none rounded py-1.5 w-full font-medium transition-colors"
        >
          Done
        </button>
      )}
